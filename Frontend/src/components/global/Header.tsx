...
export default function Header() {
    ...
    const { authorized, login } = GlobalStores.me()
    ...
    return (
        <motion.header 
            ...
            <div>
                {
                    authorized ? (
                        <Link href='/profile' className="border-1 border-white/10 nav-a rounded-full bg-[#ff1111]/10 hover:border-[#ff0000]/40 gap-2 backdrop-blur-[2px]">
                            <FaRegUserCircle/>
                            <p>{login}</p>
                        </Link>
                    ) : (
                        <Link href="/login" className="border-1 border-white/10 nav-a rounded-full bg-[#ff1111]/10 hover:border-[#ff0000]/40 gap-2 backdrop-blur-[2px]"><FaUserAstronaut/>Login</Link>
                    )
                }
