...
export default function Project( props : ProjectScheme ) {
    ...
    return(
        <div className={`flex ${props.rotate === 'left' ? ('justify-start') : ('justify-end')} main-page-project`}>
            <div className="flex project-content-container gap-6">
                {/* { props.rotate === 'left' &&  */}
                <motion.div initial={{x: -20, opacity: 0}} whileInView={{x: 0, opacity: 1}} transition={{duration: 0.4, delay: props.start_delay}} 
                    className={`${props.rotate === 'right' && "md:invisible"} bg-red-500/10 rounded-xl project-left-img`}>
                ...
                <div className={`flex flex-col main-project-text-container gap-1 ${props.rotate === 'left' ? ('items-start') : ('items-end')}`}>
                    ...
                    <motion.div initial={{x: 20, opacity: 0}} whileInView={{x: 0, opacity: 1}} transition={{duration: 0.4, delay: props.start_delay + 0.6}}>
                        <button onClick={() => handleNavigation(props.link)} className="flex gap-2 py-2 px-6 text-2xl bg-red-800/60 w-fit mt-2 items-center arrow_btn shadow-[0_0_6px_1px] shadow-red-800">Подробнее<MdOutlineDoubleArrow/></button>
                    </motion.div>
                </div>

                { props.rotate === 'right' &&
                    <motion.div initial={{x: 20, opacity: 0}} whileInView={{x: 0, opacity: 1}} transition={{duration: 0.4, delay: props.start_delay}} className="bg-red-500/10 rounded-xl main-project-img">
                        <motion.img initial={{x: 25, opacity: 0}} whileInView={{x: 0, opacity: 1}} transition={{duration: 0.4, delay: props.start_delay + 0.2}} src={props.image} width={200} className="p-6"/>
                    </motion.div>
                }   
            </div>
